...
> **Keep in sync with [`AGENTS.md`](AGENTS.md).** This repo maintains two guidance files with identical content — `CLAUDE.md` for Claude Code and `AGENTS.md` for other AI agents (GitHub Copilot CLI, Cursor, etc.). When you change one, apply the same change to the other so they never drift.
